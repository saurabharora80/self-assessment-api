...
import uk.gov.hmrc.selfassessmentapi.repositories.domain.{MongoTaxDeducted, _}
...
class FunctionalLiabilitySpec extends UnitSpec {
  ...
  "MongoLiability.toLiability" should {
    ...
    "map to liability" in {
      ...
      val liability =
        new Liability(BSONObjectID.generate,
                      "",
                      saUtr = SaUtr(""),
                      taxYear = TaxYear(""),
      ...
      liabilityDto.income.incomes.nonSavings.employment should contain theSameElementsAs Seq(
      ...
      liabilityDto.income.deductions.get shouldBe domain.Deductions(personalAllowance = 3000,
      ...
      liabilityDto.taxDeducted.fromEmployments shouldBe Seq(
        UkTaxPaidForEmployment(sourceId = "empTaxPaid", taxPaid = 20))
