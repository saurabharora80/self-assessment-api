...
class TotalIncomeCalculationSpec extends UnitSpec with SelfAssessmentSugar {
  ...
  "run" should {
    ...
    "calculate total income" in {
      ...
      TotalIncomeCalculation.run(SelfAssessment(), liability).getLiabilityOrFail shouldBe liability
        .copy(nonSavingsIncomeReceived = Some(4375), totalIncomeReceived = Some(7625))
    }
    ...
    "calculate total income if there are no sources" in {
      ...
      TotalIncomeCalculation.run(SelfAssessment(), liability).getLiabilityOrFail shouldBe liability
        .copy(nonSavingsIncomeReceived = Some(0), totalIncomeReceived = Some(0))
    }
    ...
    "calculate total income if there are employments source" in {
      ...
      TotalIncomeCalculation.run(SelfAssessment(), liability).getLiabilityOrFail shouldBe liability
        .copy(nonSavingsIncomeReceived = Some(74.5), totalIncomeReceived = Some(74.5))
    }
    ...
    "calculate total income if there are self employments source" in {
      ...
      TotalIncomeCalculation.run(SelfAssessment(), liability).getLiabilityOrFail shouldBe liability
        .copy(nonSavingsIncomeReceived = Some(500.5), totalIncomeReceived = Some(500.5))
    }
    ...
    "calculate total income if there are interest from UK banks and building societies" in {
      ...
      TotalIncomeCalculation.run(SelfAssessment(), liability).getLiabilityOrFail shouldBe liability
        .copy(nonSavingsIncomeReceived = Some(0), totalIncomeReceived = Some(400))
    }
    ...
    "calculate total income if there are dividends from unearned income" in {
      ...
      TotalIncomeCalculation.run(SelfAssessment(), liability).getLiabilityOrFail shouldBe liability
        .copy(nonSavingsIncomeReceived = Some(0), totalIncomeReceived = Some(3000))
    }
